...
- Run an idea gate before implementation; prefer one explicit single-variable idea over broad simultaneous changes.
...
- In campaign mode, rank ideas separately from run variants. Idea ranking should prefer `expected_upside`, `single_variable_fit`, and `rollback_ease`, while penalizing `implementation_risk`, `eval_risk`, and `estimated_runtime_cost`.
